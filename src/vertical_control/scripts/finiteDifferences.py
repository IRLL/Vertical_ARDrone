...
def finiteDifferences(data, numRollouts):
    ...
    # for h in range(numRollouts-1):
    for h in range((numRollouts-1)/2):
        for p in range(np.size(data[h].policy.theta)):
            # perturb = data[h+1].policy.theta[p]-data[0].policy.theta[p]
    ...
    for h in range((numRollouts-1)/2):
        ...
        plus = np.sum(data[2*h+1].r)-np.sum(data[0].r)
        minus = np.sum(data[2*h+2].r)-np.sum(data[0].r)
        # dJ[h] = np.sum(data[h+1].r)-np.sum(data[0].r)
