...
def createSys(nSystems, poliType, baseLearner, gamma):
    ...
    for i in range(nSystems):
        ...
        param.N = 4#3 # Number of states
        param.M = 2#3 # Number of inputs
        ...
        if param.disturbance[0] == 0.0 and param.disturbance[1] == 0.0:
            # x gets disturbance
            param.disturbance[0] = 0.0*np.random.uniform(-0.1,0.1,size=None) #round(np.random.rand()/2*0.01, 4)
            # y gets disturbance
            param.disturbance[1] = 0.0*np.random.uniform(-0.1,0.1,size=None)
